...
if ((imgWid % 8 != 0) || (imgHei % 8!=0)) {
  console.log('Error: Please update width or height value, it must divide by 8');
  return false;
}
  ...
  glob(__dirname + '/img/*.jpg', function(er, files) {
    ...
    if (files.length > 0) {
      readMultipleFiles(files, function(err, bufs){
        ...
        console.log('Its not died, please wait a while');
...
function redrawByOrder(frameArr) {
  ...
  function loopCheckNext() {
    ...
    if (bigestNum == -1 && storeDataArr.length == frameArr.length) {
      console.log('Frame array sort done');
...
function startDrawMergePic(orderFrameArray) {
  ...
  orderframe.forEach(function(e, index) {
    ...
    e.forEach(function(unit) {
      ...
      orderIndex++;
      console.log('Draw image done ' + parseInt(orderIndex/allLength*100) + '%');
